    ...
    while(r-l>0){
        ...
        for(int i=0; i<=n-k; i++){
            ...
            for(int j=1; j<=m-k; j++){
                ...
                found = (found || us.find(subgrid[j]));
                us.insert(subgrid[j]);
                if(subgrid[j] != revsubgrid[j]){
                    found = (found || us.find(revsubgrid[j]));
